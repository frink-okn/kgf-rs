...
plus a second independent KGF endpoint, then passes both versioned `/tpf`
URLs to `test.mjs`. The script verifies ordinary TPF discovery/paging, a bind
join that uses Comunica's brTPF `values=` transport, typed-literal ingress,
stable skolem identity, control/data graph separation, N-Quads negotiation,
and federation across the two endpoints.
