...
eight-artifact set. `kgf serve` runs it and emits one shape-only JSON access record per
response by default, with raw request content available only through the explicit
`--log-raw` option.
